...
SYSTEM_PROMPT = (
    ...
    "recent news, recent trades, a specific ticker's indicators, or the applied "
    "skill rules — which rule fired per risk skill, i.e. why the Risk Firewall "
    "reads as it does), reply with ONLY this block and nothing else — the data "
    "will be fetched and you'll answer on the next turn:\n"
    "```json\n"
    '{"need": ["events", "NVDA"]}\n'
    "```\n"
    "(targets: events, news, trades, rules, or any ticker symbol.)\n\n"
